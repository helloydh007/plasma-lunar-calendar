...
**发布时间**：国务院办公厅通常在**上一年 11 月上旬**发布次年的放假安排。历年规律：
...
```bash
python3 tools/verify-holidays.py
```

它会打印一张**对照表**（每个节日的放假区间、补班日、天数，都带星期几），便于与通知
原文逐条核对；同时自动检查：

- 放假与补班无重叠
- 补班日全部落在周末（调休补班在实践中都是周末）
- 每个节日的放假区间落在其天文/政策窗口内（春节必在 1/21–2/21 之间、清明必在 4/4–4/6
  之间、元旦必含 1/1、国庆必含 10/1……）
- 每个节日的放假是**一整段连续日期**（录入时写错月/日几乎必然造成断档）
- 各节日放假天数与年度总天数在合理区间
- `COVERED_YEARS` 与实际数据一致

已用注入错误的方式验证过它能抓到：区间断档、补班写成工作日、放假补班重叠、漏掉法定日。

**但它不是万无一失的**：它无法判断某段区间是否恰好与通知一致（例如"国庆多录一天
10/8"这类错误不会被发现）。所以**对照表必须与通知原文人工核对**——工具只是把
"核对星期几"这类机械工作自动化。
